...
def get_model_config(model_name, embed_dim=0):
    """Get a yacs CfgNode object with default values."""
    # Return a clone so that the defaults will not be altered
    # This is for the "local variable" use pattern
    config = _C.clone()
    config.MODEL.NUM_CLASSES  = embed_dim
    from pathlib import Path
    _MODEL_CONFIG_PATHS = Path(__file__).parent / f"configs/vssm1"
    yaml_file = os.path.join(_MODEL_CONFIG_PATHS,model_name+'.yaml')
    _update_config_from_file(config, yaml_file)
